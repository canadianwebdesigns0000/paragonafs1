...
// Include PHPMailer
require 'auth/assets/PHPMailer/PHPMailerAutoload.php';
...
try {
    ...
    $mail->Username = 'info@paragonafs.ca'; // Your SMTP email
    $mail->Password = 'eqdqaluvdmxnpklo';  // Your SMTP password
    $mail->SMTPSecure = 'tls';          // Enable TLS encryption
    $mail->Port = 587;                  // TCP port to connect to

    // Email sender & recipient
    $mail->setFrom('info@paragonafs.ca', 'Paragon AFS');
    $mail->addAddress('zam.canadianwebdesigns@gmail.com');  // Add a recipient

    // Email subject & body content
    $mail->isHTML(true);  // Set email format to HTML
    $mail->Subject = 'Test email from PHPMailer using SMTP';
...
if (!$mail->send()) {
    ...
    error_log("Client " . encrypt_decrypt('decrypt', $_SESSION['email']) . " Experienced Error: " . $mail->ErrorInfo);
} else {
    $response = array(
        'status' => 'success',
